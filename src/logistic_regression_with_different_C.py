...
parser = argparse.ArgumentParser(description='Train Logistic Regression on extracted skin-lesion features')
...
pd.DataFrame({
    ...
    'hyperparam': best_C,
}).to_csv(f'./results/predictions/cv_folds_LR_{args.tag}.csv', index=False)
print(f"CV fold scores saved to results/predictions/cv_folds_LR_{args.tag}.csv")
...
joblib.dump(final_lr, f'./results/models/logreg_model_{args.tag}.pkl')
...
results_df.to_csv(f'./results/predictions/predictions_LR_{args.tag}.csv', index=False)
print(f"\nPredictions saved to results/predictions/predictions_LR_{args.tag}.csv")